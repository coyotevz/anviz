...
def crc16(msg):
    ...
    for i in msg:
        crc = crc ^ ord(i)
        crc = (crc >> 8) ^ _crc_table[crc & 255]

    retval = pad_hex(crc, 4)
    return retval[2] + retval[3] + retval[0] + retval[1]

def pad_hex(d, padding=2):
    h = hex(d)[2:]
    return h.rjust(padding, '0')


def build_request(device_id, cmd, data=''):
    req = 'A5'
    req += pad_hex(device_id, 8) + hex(cmd)[2:]
    req += pad_hex(len(data), 4)
    if data:
        req += data

    req += crc16(binascii.unhexlify(req))

    return binascii.unhexlify(req)
